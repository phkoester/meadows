...
/// 
/// Whether or not the Rust executable is a test executable, makes a difference. Test executables are
/// expected to be run via Cargo, and configuration files for tests are only looked for at a few well-defined
/// idiomatic locations within the file-system tree of the crate. Standard executables, however, may both
/// be run by Cargo or "out in the wild", their invocation paths may differ from their canonicalized paths,
/// and they must support a wide range of configuration-file locations.
/// 
...
/// 
/// The following variables are set for standard executables only:
/// 
/// Name       | Description
/// :--------  | :----------
...
/// 
/// `exec_type` must be set to [`ExecType::Standard`]. The following paths are checked, in this order:
/// 
/// Path                                                 | Note
/// :--------------------------------------------------- | :---
...
/// Returns
/// 
/// - [`ConfigError::CargoManifestDirNotFound`] if `exec_type` denotes a test executable and the environment
///   variable `CARGO_MANIFEST_DIR` is not set;
...
/// );
/// ```
#[rustfmt::skip] // Protect the markdown tables
pub fn find_config_file(
  ...
  let is_test = exec_type.is_test();
  if set_env_vars {
    self::set_env_vars(is_debug, is_test);
  }
  ...
  if let Some(paths) = paths {
    for path in env::split_paths(paths) {
      if !is_test {
        tasks.push((path, vec![InvName, Name]));
      } else {
        tasks.push((path, vec![TestName]));
      }
    }
  }
  ...
  if is_test && manifest_dir.is_none() {
...
fn set_env_vars(is_debug: bool, is_test: bool) {
  static VAL: OnceLock<()> = OnceLock::new();
  VAL.get_or_init(|| set_env_vars_impl(is_debug, is_test));
}

fn set_env_vars_impl(is_debug: bool, is_test: bool) {
  ...
  if !is_test {
    // For tests, the invocation is irrelevant
